...
namespace BlobAccessTiersApp
{
    class Program
    {
        ...
        public static async Task StartContainersDemo()
        {
            ...
            BlobContainerClient containerReference = blobClient.GetBlobContainerClient(appSettings.ContainerName);

            //Get a reference for the blob
            BlobClient blobReference = containerReference.GetBlobClient(BlobFileName);
            //Get current Access Tier
            BlobProperties blobProperties = await blobReference.GetPropertiesAsync();
            System.Console.WriteLine(
                $"Access Tier: { blobProperties.AccessTier}\t" + $"Inferred: { blobProperties.AccessTierInferred}\t" + 
            ...
            System.Console.WriteLine(
                $"Access Tier: { blobProperties.AccessTier}\t" + 
